var LOG = require('logging').getLogger('http');
var debugLevel = 0;
...
//if(parser.socket.fd === null) throw new Error("Making parser\n"+sys.inspect(parser));
    parser.field = null;
    parser.value = null;
  };
...
function Client ( ) {
  ...
  var requests = [];
  var currentRequest;
  //currentRequest = "POISON";

  var parser = parsers.alloc();
  var the_test;
  //if(('socket' in parser) && parser.socket.port == 1) {
  the_test = ('socket' in parser) && parser.socket.port == 1;
  the_test = ('socket' in parser);
  the_test = ('headers' in parser);
  parser.reinitialize('response');
  if(the_test) {
    throw new Error("Parser already has a socket\n"+sys.inspect(parser.socket));
  }
  if(parser.onIncoming) {
    //throw new Error("Parser already has onIncoming\n" + I(parser));
  }
  parser.socket = this;

  var t = function(obj) {
    if(parser.socket !== self) {
        throw new Error("Client._pushRequest error\nparser.socket = " + sys.inspect(parser.socket) + "\nself = " + sys.inspect(self));
        //throw new Error("_pushRequest parser.socket.fd is null:\n"+sys.inspect(self));
    }
    /*
    var a = currentRequest, label = 'currentRequest';
    if(obj) { a = obj; label = 'Supplied object'; }
    if(!a) throw new Error(label+" is non-true");
    if(a == 'POISON') throw new Error(label+" is uninitialized");
    */
  };

  var next_cur = function next_cur() {
    LOG.debg("next_cur with "+requests.length + '\n' + I(requests));
    //+sys.inspect(requests).replace(/\n/g, ' '));
    if(requests.length == 0) throw new Error("Requests is empty!");
    currentRequest = requests.shift();
  };

  self._reconnect = function () {
    if (self.readyState != "opening") {
      //LOG.debg("HTTP CLIENT: reconnecting readyState = " + self.readyState);
      self.connect(self.port, self.host);
    }
  };

  self._pushRequest = function (req) {
    LOG.debg("will _pushRequest with " + requests.length + ' existing\n' + I(req));
    req.addListener("flush", function () {
      if (self.readyState == "closed") {
        //LOG.debg("HTTP CLIENT request flush. reconnect.  readyState = " + self.readyState);
        self._reconnect();
        return;
      }

      //t();
      LOG.debg("self flush  readyState = " + self.readyState);
  ...
  self.ondata = function (d, start, end) {
    LOG.debg('ondata:\n'+d.slice(start, end));
    var bytesParsed = parser.execute(d, start, end - start);
    LOG.debg("bytesParsed = "+bytesParsed);
    if (parser.incoming && parser.incoming.upgrade) {
      LOG.debg("YES");
    ...
    else { LOG.debg("NOT ENTERED"); }
  };

  self.addListener("connect", function () {
    LOG.debg('connect');
    parser.jason = {host:this.host, port:this.port, url:(parser.incoming ? parser.incoming.url : 'no parser.incomgin')};
    if (this.https) {
	this.setSecure(this.credentials);
    } else {
      parser.reinitialize('response');
      LOG.debg('requests: ' + requests.length);//sys.inspect(requests));
      next_cur();
  ...
  self.onend = function () {
    LOG.debg('onend');
    parser.finish(); // XXX Maybe this is not closing down sufficiently?

    LOG.debg("self got end closing. readyState = " + self.readyState);
    self.end();
  };

  self.addListener("close", function (e, extra) {
    LOG.debg('close '+sys.inspect({e:e, extra:extra, parser:parser}));
    /*
    if (e) {
      self.emit("error", e);
      return;
    ...
    */
    ...
    LOG.debg("HTTP CLIENT onClose. readyState = " + self.readyState);

    // If there are more requests to handle, reconnect.
    if (requests.length > 0) {
      LOG.debg("Reconnecting\n" + I(requests));
      self._reconnect();
    } else {
      LOG.debg("Freeing:\n"+I(parser));
      //parsers.free(parser); // XXX: If this is not freed then the bug is not triggered.
    }
  });

  parser.onIncoming = function (res) {
    //if(res.socket.fd === null) {
    /*
    if(res.socket.port == 1) {
      var i = I({res:res, currentRequest:currentRequest, port:self.port});
      LOG.debg('detected res.socket.port == 1\n' + i);
      setTimeout(function() {
      throw new Error("This is bad\n" + i);
      }, 250);
    }
    */
    LOG.debg("incoming response:\n" + sys.inspect(res));

    res.addListener('end', function ( ) {
      LOG.debg("request complete disconnecting. readyState = " + self.readyState);
      self.end();
    });

    if(!currentRequest) {
      throw new Error("emit will fail:\n" + I({res: res, currentRequest:currentRequest, self:self}));
      //throw new Error("...");
    }
    else
      LOG.debg("emit will not fail");
    currentRequest.emit("response", res);
  };
  LOG.debg("Assigning to parser.socket: " + sys.inspect({parser:parser, 'this':this}));
};
